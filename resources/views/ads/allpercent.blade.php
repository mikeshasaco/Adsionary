...
<div class="container">
    <div class="row">
             <div class="col-md-12 col-sm-12">
                    <h4 style="text-align: center" >Most used Discount Percentages for Advertisements According to our Database </h4>
                ...
                <?php $cats = DB::table('industries')->orderby('industryname','ASC')->get(); ?>
                 ...
                 <div class="selectagcategory">
                  <select onchange="if (this.value) window.location.href=this.value" style="margin-top:5%; " class="media-select">
                        ...
                        <option value="{{ url('/data/industry/percentage'.'/'. $categoryselect->id)  }}">{{ $categoryselect->industryname }}</option>
                    @endforeach
                    </select>
                </div> 
  ...
  <div class="skills" >
      @foreach($percent as $type)
      <div class="skill">
        <div class="skill-name"><b> {{number_format($type->total,0)}}% </b>   of  Advertisement in our database use <b>{{$type->percentnumber}} Percent OFF </b> </div>
        <div class="skill-bar">
        @if($type->percentage_id == 1)
        <div class="skill-per" style="background-color: cadetblue" per="{{$type->total}}"></div>
        @elseif($type->percentage_id == 2)
        <div class="skill-per" style="background-color: rgb(203, 23, 164)" per="{{$type->total}}"></div>
            @elseif($type->percentage_id == 3)
        <div class="skill-per" style="background-color: rgb(108, 58, 216)" per="{{$type->total}}"></div>
                @elseif($type->percentage_id == 4)
        <div class="skill-per" style="background-color: burlywood" per="{{$type->total}}"></div>
                @elseif($type->percentage_id == 5)
        <div class="skill-per" style="background-color: rgb(44, 132, 64)" per="{{$type->total}}"></div>
        @else
        <div class="skill-per" style="background-color: rgb(204, 63, 63)" per="{{$type->total}}"></div>
            @endif
        </div>
        @endforeach
      </div>
