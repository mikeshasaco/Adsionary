...
<div class="container">
    <div class="row">
             <div class="col-md-12 col-sm-12">
                    <h4 style="text-align: center" >Most used Social Medias Type for Advertisements According to our Database </h4>
                ...
                <?php $cats = DB::table('industries')->orderby('industryname','ASC')->get(); ?>
                 ...
                 <div class="selectagcategory">
                  <select onchange="if (this.value) window.location.href=this.value" style="margin-top:5%; " class="media-select">
                        ...
                        <option value="{{ url('/data/industry/socialmedia'.'/'. $categoryselect->id)  }}">{{ $categoryselect->industryname }}</option>
                    @endforeach
                    </select>
                </div> 
  ...
  <div class="skills" >
      @foreach($social as $type)
      <div class="skill">
        <div class="skill-name">{{number_format($type->total,0)}}% of Advertisements used {{$type->socialmedia}} </div>
        <div class="skill-bar">
          @if($type->social_id == 1)
